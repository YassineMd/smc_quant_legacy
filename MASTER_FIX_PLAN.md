...
**DAEMON FIX — bucket closes weren't broadcasting at the cap (`3384a6b`).** A real production bug, found while
chasing a "live candles leave a gap / only a restart fixes it" report. ROOT CAUSE: `feeds.py`'s per-close
`ObPacket` broadcast fired only `if len(engine.closed_buckets) > last_bucket_count`, but the engine caps that
list at `CLOSED_BUCKETS_CAP` (10k) via append+pop(0) — so once at the cap the length stops growing and the
`>` check NEVER fires again. Closes stopped broadcasting; the client's scanner history froze at the last
catch-up (a terminal restart re-pulls a fresh catch-up → "fixed" briefly, then re-freezes). Latent for days
(daemon crossed 10k long ago); coincided with the cloud work by accident. FIX: a MONOTONIC `engine.total_closed`
counter (incremented at the append, never trimmed); `feeds.py` detects closes by its delta and ships
`closed_buckets[-delta:]`. Byte-identical below the cap; only fixes the broken at-the-cap case. PROVEN on the
live VM (after deploy): 8 closes broadcast in 160s, the client tail advanced (was frozen). **`feeds.py:389`
was the SOLE instance of the length-growth pattern** (a systemic grep confirmed other `len()` checks are
warmup guards or full-list catch-up ships). Method that found it: revert-and-check (ruled out the cloud) +
a live-edge monitor. Deployed via `deploy.ps1` + `systemctl restart orderflow` (10-day history preserved).
**Two earlier wrong guesses (blamed the cloud, then cloud-perf) were corrected by empirical proof** — see
[[demands-empirical-proof]].

**Mode-10 SELECTION EXHAUSTION lines — gated, fresh-from-selection, BUILT (`fe0a714`).** Two smoothed lines
(blue=bull / red=bear) showing each side's exhaustion across the buckets INSIDE a selection, in a panel BELOW
the box (outside it — never covers the candles), 0/50/100% scale, gold diamonds at crossovers (exhausted side
swaps). `'1'` toggles; hover gives the RAW per-bucket %.
- **The audits that preceded it (read-only):** the RAW per-bucket effort-z (`b_mult`/`s_mult`) is NOT saturated
  — centered ~0.94, full range, ~30% low; the 96%-at-flips figure was a loose binary, not the continuous value.
  The GATED state (`geomean(absorb, drain/cover, Δsoft)`) is the TRUE worn-out signal: effort hot AND OI
  draining → it separates worn-out from strong-continuation by **36×** (continuation 0.016 vs true-exh 0.578),
  but it's sparse/spiky (~6-11% nonzero). A chart-wide Keltner cloud of the gated score was built then SCRAPPED
  (operator wanted it selection-scoped, not around every candle).
- **MEASURE = GATED** (operator's pick over raw, shown side-by-side on real data): raw is a smooth both-sided
  weave (matches a hand-drawing) BUT mislabels a *winning* side as exhausted (sellers driving price down read
  "95% exhausted"); gated stays 0% there (OI building = winning, not worn out) and only fires on genuine
  worn-out. `config.EXH_MEASURE = "gated"|"raw"` (gated default). `region_state.selection_exhaustion`.
- **BASELINE = FRESH FROM SELECTION START:** each bucket's E/R z uses ONLY the prior buckets WITHIN the
  selection (expanding window, min `EXH_SEL_MIN_WINDOW`=2; bucket 0 neutral) — never reaching before the
  selection. Selection-relative by design (same bucket reads differently per where the box is drawn; a uniform
  trend self-normalizes to ~0 — exhaustion shows on a bucket abnormal vs its *neighbors*, not "the move went
  down"). `_exh_z_mult` gained a `min_window` param for the thin early baseline.
- **INTERPRETATION (recorded for the operator):** bull = buyers worn out from pushing UP (a top thing), bear =
  sellers worn out at bottoms. In a down-move, BULL 0% is correct (buyers aren't the pushing side; bull only
  scores on up-close buckets). Both 0% in a downtrend = "no one's worn out → trend has fuel, no reversal
  warning." A bull-exhaustion spike + bear 0% in a downtrend = "the bounce is out of buyers while sellers are
  fresh → trend likely resumes" (NOT a bottom; the bottom is the mirror — bear spiking, bull fresh).
- **DISPLAY:** raw per-bucket → SYMMETRIC envelope smoothing (`max(forward, backward)` so a crossover marks the
  true balance shift, not a causal decay tail) → panel-mapped (0% bottom, 100% top). Panel sits `EXH_STRIP_GAP`
  below the box, height `EXH_STRIP_FRAC` of the selection's price range, **no dark backing** (chart's already
...
- **2b is BUILT + the live-price latency bug it surfaced is DEPLOYED — see the two entries below. Next: Phase 3 bubbles.**
...
(`depth_heatmap_state`) and NEVER the 20Hz `snapshot()` — zero cost when the mode is closed.
- **`app/heatmap.py`** (NEW, pure / no-Qt, headlessly tested): base64 float32 decode; `HeatmapCache` (contiguous
  time-ordered columns — O(1) live append, lazy CONTIGUOUS prepend on scroll-back, FREE re-slice on a pan inside
  the loaded range, time-span trim); percentile contrast; the neon diverging LUT + side-sign.
- **Render:** `ImageItem(col-major)` over a SIGNED pre-log grid (±log10(size+1), signed by side); adaptive
  contrast (percentile cutoffs + 60s renorm) via the `HeatmapContrastBar` sliders (default **p98.9 / p99.4** —
  strongest liquidity first). Smooth 20Hz view-follow DECOUPLED from the data-change rebuild.
- **NEON palette by SIDE (Deepdom):** bids/buy-limits = green, asks/sell-limits = purple, intensity by size,
  TRANSPARENT below the cutoff (the dark canvas shows, no blue wash). Greyscale 'g'.
- **BBO:** solid FORMED trace ending at the live edge + DASHED current bid/ask projecting forward into the
  un-formed future (Bookmap LLT) + Y-axis price tags; crosshair shows time + price.
- **Scan-Start HARD floor** (never request/back-fill before the chosen start); the DATA window never extends
  past 'now' (blank to the right = empty view — no carry-forward flat lines, no force-back on pan). **Tick-
  aligned ybins** (one bin/tick → a level fills its cell, not a hairline).
- **DOM ladder (`cob_panel`):** default-ON in heatmap mode, neon-recolored, price-bucket AGGREGATION tracking
  the heatmap row height (coarsens zoomed out, refines to 1 tick zoomed in), crosshair-driven price/size readout.
- **Validated:** compile/import/suite 9/9 + headless (decode/axis-order, cache stitch/ring/contiguity, LUT,
  signed-cache, cob aggregation/mark). **Next: Phase 3 = executed-trade bubbles** (green=buy / red=sell, size=qty).

**🟢 LIVE-PRICE LATENCY FIX — recompute was starving the broadcast loop (`ad4d467`, ✅ DEPLOYED + VERIFIED on
`smc-quant-eu` 2026-06-26).** Surfaced during the 2b eyeball: live price "a few seconds behind, catches up in
bursts" in BOTH heatmap AND bucket modes. PROVEN on the VM (probe of the daemon's own output, not guessed): a
fresh 1m subscriber got 1 frame in 10s while the daemon burned ~70% of a core; the single asyncio loop blocked
3-4s every ~5s (=`RECOMPUTE_SECS`). Root cause: `recompute_loop` + the 19.3b live-edge refresh rescanned OBs AND
serialized the forming footprint / OB matrix for ALL 5 tfs every cycle (`to_line()` runs BEFORE `broadcast_tf`
filters by subscription), and `calc_quant_obs` is O(obs×buckets) (~3s on a ~10k-bucket 1m engine — the
mitigation loop).
- **Fix (two levers):** (1) **subscription-gating** — only compute/serialize/broadcast per-tf work for tfs a
  client subscribes to (`daemon.tf_has_subscribers` → `core._tf_subbed`); one terminal on 1m does 1m work, not
  5×. (2) **skip-if-unchanged** — recompute only when `engine.total_closed` moved since its last run; the OB set
...
- **Next: Step B = Phase 3 trade bubbles** on the clean daemon — ✅ DONE, see below.

**🟢 HEATMAP Phase 3 — executed-trade bubbles + iceberg overlay + heatmap UX polish (`40d2a69`, BUILT + EYEBALLED on SOLUSDT).** Mirrors the proven `depth_window` pipeline so the single event loop is never blocked. Terminal-side polish needs NO redeploy; the daemon trades endpoint deploys with `deploy.ps1`+restart.
- **Daemon/transport — PROVEN zero new loop-blocking (gap probe: max 0.26s vs clean 0.37s baseline); lossless bit-identical on real VM data (7780 trades).** `depth_store.trades_window` (read-only `mode=ro`, ts_ms index) → `_send_trades_window` via `run_in_executor` (off-loop) + `trades_live_loop` (batched push per pulse, O(1) capture-tee). `protocol`: `TradesWindowPacket`/`TradeBatchPacket` = 4 base64 arrays (ts int64, price/qty **float64 bit-identical** to trade_tape, side uint8). `pipe_client` dedicated `trades_state` delivery buffer — NEVER on the 20Hz `snapshot()` (the isolation guarantee).
- **Bubbles (`heatmap.py` `TradeBubbleCache.visible_cells` + `_hm_render_bubbles`):** numpy bin/aggregate the visible trades into (col×bin) cells → size by total qty, color by net side. Two `pxMode` scatters (green=buy / purple=sell), area∝qty (√ radius + px clamp), `b` toggles, min-qty declutter. Hover pill via `sigHovered` (black text on the matching neon fill); `tip=None` kills pyqtgraph's built-in x/y/data box.
- **Iceberg overlay:** cells sitting on an active absorption level recolor **ELECTRIC BLUE (buy) / ORANGE (sell)**, classified from `snap['absorptions']` by price band — the SAME standing levels the bucket chart's `AbsorptionLayer` draws (per-tf, so "1m" follows the daemon's tf).
- **Polish:** fine **per-pulse BBO trace** (separate from the binned grid image) so the best-bid/ask lines follow the live price instead of lagging the bubbles on a fast move · **shift+hover** resting-liquidity readout (`HeatmapCache.raw_at`, neighbour-search forgiving) · DOM ladder in-view `cob.autoscale_x` (a far wall can't flatten the zoomed-in ladder) · follow **15% lead gutter** (live edge at ~85%) + double-click re-lock + a manual **PAN detaches follow instantly** (width-unchanged) while a zoom keeps it · Liquidity Contrast panel → extreme top-left · scanner menu **"Heatmap"** label, listed **2nd** · heatmap-open auto-cancels any armed draw tool (`DrawingController.cancel()` now un-highlights the toolbar button).
- **Verify:** per-cell aggregation + `raw_at` + fine-BBO + iceberg-classification unit tests pass; relevant regression suite green. **Phase 2b heatmap + Phase 3 bubbles = the Bookmap-style liquidity overlay is FEATURE-COMPLETE; eyeballed live across several iterations.**
