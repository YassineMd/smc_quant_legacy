...
### Step 3 — Honest churn decomposition (engine; schema change — atomic)
**Problem:** The `churn / 2` split fabricates open-vs-close positioning out of volume that *did not change OI*. There is no L1 information to attribute it; the 50/50 is pure invention, and it makes your position curves look busy in ranging markets where nothing structural is happening.
**Directive:**
- Redefine the 4 vectors to carry **only the OI-confirmed portion**: when `delta_oi > 0`, split `delta_oi` into `opL/opS` by taker ratio; when `delta_oi < 0`, split `|delta_oi|` into `clL/clS` by taker ratio. Remove the `churn/2` terms entirely. `# DIVERGES FROM LEGACY`.
...
- Update Modes 1/2/7/8 to render the confirmed vectors plus a neutral churn band. Update Mode 3's derived `delta_oi` (`(opL+opS) - (clL+clS)`) — it now equals true OI delta exactly, which is more correct.
**Invariants:** atomic schema update across protocol/engine/persistence/terminal (0.3).
**Verify:** 5-component conservation holds; render the same historical window before/after and confirm the curves go appropriately quiet in ranging periods and only build during real OI expansion. **This is a meaning change — eyeball it on a real session before trusting it.**
...
## PHASE 5 — The architectural decision (the fidelity ceiling)

### Step 19 — kline → aggTrade for footprint/level fidelity (SCOPED DECISION)
**The honest truth:** Steps 1–18 make the system *internally correct and honest given kline data*. But footprints, imbalances, icebergs, per-level POC, and OB bands are reconstructed from one close-price-per-push. They will remain *approximations of the tape* no matter how clean the math is. Your **scalar** modes (volume, CVD, VPIN, OI-based vectors) run on accurate aggregate data and are fully trustworthy after Phase 1 — those do **not** need this. Only the **level-distribution** signals do.
**Directive (decide first — this is a real migration, not a step to paste blind):** Add an `@aggTrade` (or `@trade`) websocket in `feeds.py`; route each trade (true price + qty + buyer-maker flag) into the footprint levels and `process_tick`, with klines retained only for OHLC/candle framing and OI alignment. This raises message volume by orders of magnitude — the per-tick hot path must stay allocation-light. Treat it as its own mini-project with its own plan and its own before/after validation (compare an aggTrade-built footprint against the kline-built one for the same window).
