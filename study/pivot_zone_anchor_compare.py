"""Zone-anchor check: does classifying the zone by the ENTRY price (E2 / E-held, where the trade actually fills)
vs the DETECTION price (D) change the results? For each frozen trade compute zone_by_D (cl[det] vs the 4h bucket
at det) AND zone_by_ENTRY (cl[j0] vs the 4h bucket at j0). Report: how often they disagree, the zone marginals
(three-outcome) under each anchor side by side, and the composite TAKE filter under each anchor.
Run: python study/pivot_zone_anchor_compare.py
"""
...
def main():
    ...
    for f in fires:
        ...
        if zD is None or zE is None:
            continue
        rows.append(dict(g=walk(det, j0, buy), tier=tier, buy=buy, zD=zD, zE=zE))
    ...
    ndiff = sum(1 for r in rows if r["zD"] != r["zE"])
    print("ZONE ANCHOR: D (detection) vs ENTRY (E2/E-held)  —  %d trades\n" % len(rows))
    print("  zone DISAGREES between D and entry on %d/%d trades (%.0f%%)" % (ndiff, len(rows), 100.0 * ndiff / len(rows)))
    # transition matrix (how detection-zone maps to entry-zone)
    print("\n  where trades MOVE (D-zone -> entry-zone), only when different:")
    from collections import Counter
    trans = Counter((r["zD"], r["zE"]) for r in rows if r["zD"] != r["zE"])
    for (a, b), c in sorted(trans.items(), key=lambda x: -x[1]):
        print("    %-12s -> %-12s : %d" % (a, b, c))

    for anchor in ("zD", "zE"):
        name = "DETECTION (D)" if anchor == "zD" else "ENTRY (E2/E-held)"
        print("\n  ZONE MARGINAL — anchored on %s:" % name)
        for z in ZONES:
            line(z, [r[anchor] == z for r in rows])

    # composite filter under each anchor
    def take_of(r, anchor):
        z = r[anchor]; buy = r["buy"]; tier = r["tier"]
        own_in = (z == "inzone-buy") if buy else (z == "inzone-sell")
        own_bey = (z == "beyond-down") if buy else (z == "beyond-up")
        is_body = z == "body"
        if tier == "hollow":
            return own_in or is_body
        if tier == "cyan/orange":
            return own_in
        return own_in or own_bey

    print("\n  COMPOSITE TAKE filter under each anchor:")
    for anchor in ("zD", "zE"):
        name = "D" if anchor == "zD" else "ENTRY"
        keep = [take_of(r, anchor) for r in rows]
        line("KEPT (anchor=%s)" % name, keep)
