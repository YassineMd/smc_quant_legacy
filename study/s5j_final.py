...
def main():
    ...
    # legs 1-4 — S5i verbatim (leg 2 = share form)
    ...
    def context_scan(b):
        ...
        for N in range(50, 101):
            ...
            if o < c:
                return "short"
        return None
    s5i_bars = {s: {int(idx[r]) for r in np.flatnonzero(legs14[s])
    ...
    md = ["# S5j — Final Leg-5 (EXISTS = S5d leg5w) + S5i machinery (LOCKED)", "",
          "_**Pre-registered; +2 cells -> counter 516. Supersedes S5i (its priority-scan encoding"
          " was an architect error). Leg 5 = S5d's leg5w EXISTS form, code reused verbatim"
          " (rolling zone max/min); both-eligible bars are legal, legs 1'-4 pick the side."
          " Everything else identical to S5i: share-form leg 2 (>= 65%%), 1h windows + fire-search"
    ...
    for s in ("long", "short"):
        ...
        md.append("| %s | %d | +%d / -%d (n=%d) | +%d / -%d (n=%d) | %d | %d | %d | %d | %d |" % (
            s, len(fb), len(fb - s5i_bars[s]), len(s5i_bars[s] - fb), len(s5i_bars[s]),
