...
## CANDIDATE — DA2-REVERSION v1.1 (v1.0 + QUIET-TAPE gate; runs BESIDE v1.0, not instead of it)

| id | frozen | delta vs v1.0 | status |
|---|---|---|---|
| **DA2-REVERSION-v1.1** | 2026-07-21 · freeze_ts 1784573943 (Jul20 18:59:03 UTC) · `study/out/da2_reversion_v11_freeze.json` | **v1.0 signal AND `\|eff-agg spread\| <= 50`** (non-locked / first-print / causal — the LOCKED variant repaints and is BANNED). Entry, SL 0.8%, TP 1.0% and the non-overlap contract are inherited from v1.0 unchanged. Rationale: fading a candle only works when that candle had NO conviction; a large \|spread\| means one side owned the aggressor flow (a real move, not a failed probe). Near-zero spread = the same "no conviction" story da2 detects INSIDE the bucket, confirmed independently ACROSS it. | **in-sample n=99 · win 59.6% vs 48.9% break-even · +0.1927%/tr · +19.1% · t+2.16.** Split-half BOTH positive (H1 +0.112 / H2 +0.272); drop-best-5 +0.154; MC P(profit) **98.4%**, 95%CI **[+0.011, +0.356] CLEARS ZERO** (v1.0's straddled it). **Fixes v1.0's two worst properties:** sides now BALANCED (LONG +0.214 n=51 / SHORT +0.170 n=48, vs v1.0's +0.034/+0.230 short-carry), and concentration cut — **all four weeks positive** (+2.52/+5.04/+9.56/+1.96), week-3 share 76% → 50%. ⚠ But it is a **RE-CHAINING not a subtraction**: 128→99 is 45 out / 25 in (filtering frees later signals through the non-overlap chain), and the 45 removed average **−0.0000% at 49% win** — dead neutral, NOT losers. SPREAD_MAX=50 was grid-selected from a 50/65/80 plateau (+0.193/+0.203/+0.169); permutation vs a random same-size subset p=0.021, which controls for subset SIZE but not the reshuffle nor the ~150 session cells. **v1.0 stays frozen and runs in parallel — the pair is the experiment.** Spec: `study/da2_reversion_v11_validate.py`; audit: `study/da2_reversion_v11_forward_audit.py`; log: `study/out/da2_reversion_v11_forward_log.md` (**forward n=0 at freeze**). |

**THREE DEAD ENDS on eff-agg (measured, monotone, do NOT retry):** (1) spread LEVEL aligned with the trade →
monotonically destructive, T=0 −0.054 down to T=65 −0.700 (10% win); (2) spread MOMENTUM aligned with the trade
(`ds = spr[i]−spr[i−1]`) → same shape, T=0 −0.030 down to T=20 −0.326 (30.8% win), every cell fails split-half,
and the signals naturally lean the OTHER way (LONG median ds −10.5, SHORT +11.3); (3) inverting (2) is no better
than baseline and fails split-half. **Reading: this edge lives where the tape is NOT confirming. Only the ABSENCE
of strong agreement helps — which is exactly what v1.1 gates on.**
