"""Winner/breakeven/loser rate by D-tier x 4h zone-state. D-tier = the D-badge fill = aligned panel-2 spread at
D: cyan/orange (VHI > 80) | red/green (HI 63-80) | hollow (<=63). Zone-state (detection price vs the last
completed 4h bucket's two wick bands, GREEN buy [low, vq_lo] & RED sell [vq_hi, high]):
  WITH     = in its OWN zone  (long in GREEN / short in RED)   e.g. a sell D in the sell/red area
  OPPOSITE = in the OTHER zone(long in RED  / short in GREEN)  e.g. a sell D in the buy/green area
  NEITHER  = in neither band (the body / outside the 4h range)
3 tiers x 3 states, three-outcome (winner net>+0.05%% / breakeven |net|<=0.05%% / loser net<-0.05%%) + marginals.
Frozen exit throughout. Run: python study/pivot_dtier_zone3.py
"""
...
def main():
    ...
    scan = {"long": 0, "short": 0}; rows = []; both = 0
    for f in fires:
        ...
        if i4 < 0:
            continue                                          # need a completed 4h bucket
        px = float(cl[det])
        in_green = z_low[i4] <= px <= z_lo[i4]
        in_red = z_hi[i4] <= px <= z_high[i4]
        if in_green and in_red:
            both += 1
        wth = in_green if buy else in_red                     # own zone
        opp = in_red if buy else in_green                     # opposite zone
        state = "WITH" if wth else ("OPPOSITE" if opp else "NEITHER")
        rows.append((walk(det, j0, buy), tier, state))

    G = np.array([r[0] for r in rows]); NET = G - FEE
    TIER = np.array([r[1] for r in rows]); STATE = np.array([r[2] for r in rows])
    d = 1000.0 / 100.0
    ...
    print("D-TIER x 4h ZONE-STATE  (WITH own / OPPOSITE / NEITHER)  n=%d, %d in both bands\n" % (len(rows), both))
    for tier in ("cyan/orange", "red/green", "hollow"):
        tm = TIER == tier
        print("  %s  (all: n=%d):" % (tier.upper(), int(tm.sum())))
        for stt in ("WITH", "OPPOSITE", "NEITHER"):
            line("  %s" % stt, tm & (STATE == stt))
        print()

    print("  MARGINALS by zone-state (all tiers):")
    for stt in ("WITH", "OPPOSITE", "NEITHER"):
        line(stt, STATE == stt)
    print("\n  MARGINALS by tier (all states):")
    for tier in ("cyan/orange", "red/green", "hollow"):
        line(tier, TIER == tier)
    print("\n  FILLED-D (cyan/orange + red/green) x state — the ledger overlay:")
    filled = (TIER == "cyan/orange") | (TIER == "red/green")
    for stt in ("WITH", "OPPOSITE", "NEITHER"):
        line("filled  %s" % stt, filled & (STATE == stt))
    line("ALL", np.ones(len(G), bool))
