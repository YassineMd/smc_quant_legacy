"""Composite tier x zone TAKE filter v2 — the qualifying zone can be at D OR at the ENTRY (E2/E-held); take if
EITHER anchor satisfies the tier rule. Rules (OWN follows D side: buy->buy wick/below buy zone, sell->sell wick/
above sell zone; REVERSED = the other wick):
...
Non-merged 4h wick. Frozen entries+exit; this only includes/excludes. Reports three-outcome for KEPT (D OR entry)
vs ALL vs DROPPED, plus D-only / entry-only for reference, the cut accounting, and per-tier. Three-outcome on NET.
Run: python study/pivot_composite_filter2.py
"""
...
def take_rule(zone, buy, tier):
    ...
    is_body = zone == "body"
    if tier == "hollow":
        return rev_in or is_body or own_bey
    if tier == "cyan/orange":
        return own_in
    return own_in or own_bey                                   # red/green
...
def main():
    ...
    for f in fires:
        ...
        if zD is None or zE is None:
            continue
        tD = take_rule(zD, buy, tier); tE = take_rule(zE, buy, tier)
        rows.append(dict(g=walk(det, j0, buy), tier=tier, buy=buy, tD=tD, tE=tE, take=tD or tE))

    G = np.array([r["g"] for r in rows]) - FEE
    TAKE = np.array([r["take"] for r in rows]); TD = np.array([r["tD"] for r in rows]); TE = np.array([r["tE"] for r in rows])
    ...
    print("COMPOSITE v2  (take if D OR entry qualifies)  frozen entries+exit; %d trades over %.1f days\n" % (len(rows), span))
    line("ALL (frozen, no filter)", np.ones(len(rows), bool))
    line("KEPT  D OR entry", TAKE)
    line("  (ref) KEPT D-only", TD)
    line("  (ref) KEPT entry-only", TE)
    line("DROPPED (neither)", ~TAKE)
    cut = ~TAKE
    print("\n  cut %d/%d: %d losers, %d BE, %d winners  |  KEPT %.1f trades/day (all %.1f)"
          % (int(cut.sum()), len(rows), int((G[cut] < -BE).sum()), int((np.abs(G[cut]) <= BE).sum()),
             int((G[cut] > BE).sum()), int(TAKE.sum()) / span, len(rows) / span))
    print("  KEPT $%+.0f vs ALL $%+.0f  ->  %+.0f  (per-trade net KEPT %+.3f%% vs ALL %+.3f%%)"
          % (G[TAKE].sum() * 10.0, G.sum() * 10.0, (G[TAKE].sum() - G.sum()) * 10.0,
             G[TAKE].mean() if TAKE.any() else 0.0, G.mean()))

    print("\n  per-tier (KEPT D-OR-entry vs that tier's ALL):")
    for t in ("hollow", "cyan/orange", "red/green"):
        tm = TIER == t
        line("  %-11s ALL" % t, tm); line("  %-11s KEPT" % t, tm & TAKE)
