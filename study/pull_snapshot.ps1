...
$py = "import sqlite3, os" + "`n" +
      "src = os.path.expanduser('~/OrderFlowPlatform/data/history.db')" + "`n" +
...
$mkSnap = "rm -f $TMP && echo $b64 | base64 -d | python3 -"
...
gcloud compute ssh $VM --zone $ZONE --project $PROJECT --command "rm -f $TMP"
