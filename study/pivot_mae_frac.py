"""Hypothesis: losers retrace 40-60% of the entry->SL distance early (before +0.4% arms the breakeven lock),
winners don't. Measures each trade's PRE-LOCK max adverse excursion as a FRACTION of R = |entry - initial SL|
(1.0 = hit the stop). Splits winner/loser and tabulates each fraction threshold. Run: python study/pivot_mae_frac.py
"""
...
def main():
    ...
    def walk(det, j0, buy):
        ...
        if buy:
            ...
            arm_lvl = entry * (1 - ARM); lock_lvl = entry * (1 - LOCK)
        R = abs(entry - sl0)                                           # risk distance (entry -> initial SL)
        ...
        for j in range(j0 + 1, n):
            ...
            if not armed:                                             # PRE-LOCK adverse only
                adv = (entry - lo[j]) if buy else (hi[j] - entry)
                worst = max(worst, adv)
            e = exitlvl
            if armed:
    ...
    for f in fires:
        ...
        if j0 is None:
            continue
        g, fr = walk(det, j0, buy); G.append(g); F.append(fr)

    G = np.array(G); F = np.array(F); w = G > 0
    print("PRE-LOCK MAE as a FRACTION of the entry->SL risk (1.0 = hit the stop) — %d trades\n" % len(G))
    print("  WINNERS (n=%d): frac median %.2f | mean %.2f | 90th pct %.2f | max %.2f"
          % (w.sum(), np.median(F[w]), F[w].mean(), np.percentile(F[w], 90), F[w].max()))
    print("  LOSERS  (n=%d): frac median %.2f | mean %.2f | 90th pct %.2f | max %.2f"
          % ((~w).sum(), np.median(F[~w]), F[~w].mean(), np.percentile(F[~w], 90), F[~w].max()))
    print("\n  retrace >= | #trades | win%% of  | %% of winners | %% of losers")
    print("  (frac of R)| reaching | reachers | that reach it| that reach it")
    for T in (0.2, 0.4, 0.5, 0.6, 0.8, 1.0):
        reach = F >= T - 1e-9
        winr = 100.0 * np.mean(G[reach] > 0) if reach.sum() else float("nan")
        pw = 100.0 * np.mean(F[w] >= T - 1e-9); pl = 100.0 * np.mean(F[~w] >= T - 1e-9)
        print("     %3.0f%%    |   %3d    |  %5.1f%%  |    %5.1f%%    |   %5.1f%%" % (T * 100, reach.sum(), winr, pw, pl))
