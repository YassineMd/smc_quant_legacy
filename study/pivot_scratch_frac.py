"""Scratch on a RISK-FRACTION trigger: if price retraces >=50% (or 60%) of the entry->SL distance R, arm an
exit at breakeven / +0.1% on the recovery. Measures losers-improved vs winners-trimmed and the net $ impact vs
the frozen strategy. Answers: beneficial? does it make money? Run: python study/pivot_scratch_frac.py
"""
...
def main():
    ...
    def walk(det, j0, buy, trig, scr):
        ...
        if buy:
            ...
            arm_lvl = entry * (1 - ARM); lock_lvl = entry * (1 - LOCK)
        R = abs(entry - sl0)
        retr = 0.0 if trig is None else ((entry - trig * R) if buy else (entry + trig * R))
        scrt = (entry * (1 + scr)) if buy else (entry * (1 - scr))
        exitlvl = sl0; tp = 0; armed = False; scr_on = False
        for j in range(j0 + 1, n):
            ...
            if trig is not None and scr_on and ((hi[j] >= scrt) if buy else (lo[j] <= scrt)):
                return ((scrt - entry) if buy else (entry - scrt)) / entry * 100.0
            if trig is not None and ((lo[j] <= retr) if buy else (hi[j] >= retr)):
                scr_on = True
    ...
    base = np.array([walk(d, j, b, None, 0.0) for d, j, b in trips])
    d = 1000.0 / 100.0                                             # $ per 1% at $1000/trade
    print("SCRATCH on risk-fraction retrace -> breakeven / +0.1%%  (frozen base, %d trades, $1000/trade)\n" % len(base))
    print("  FROZEN base: win %.1f%% | net %+.3f%%/trade | TOTAL %+.2f%%  ($%+.0f)\n"
          % (100.0 * np.mean(base > 0), (base - FEE).mean(), (base - FEE).sum(), (base - FEE).sum() * d))
    print("  variant                    | losers improved | winners trimmed | TOTAL  | vs base | $ vs base")
    for trig in (0.5, 0.6):
        for scr, nm in ((0.0, "breakeven"), (0.001, "+0.1%")):
            a = np.array([walk(dd, jj, bb, trig, scr) for dd, jj, bb in trips])
            imp = int(np.sum((base < 0) & (a > base + 1e-9)))
            cut = int(np.sum((base > 0) & (a < base - 1e-9)))
            tot = (a - FEE).sum(); dif = tot - (base - FEE).sum()
            print("  retrace %2.0f%% -> %-9s   |      %2d         |      %2d         | %+6.2f%% | %+6.2f%% | $%+.0f"
                  % (trig * 100, nm, imp, cut, tot, dif, dif * d))
    print("\n  ($ vs base = extra/less profit per $1000 traded per trade, over the ~9-day in-sample tape)")
