"""15mReasy variant: skew SIGN-only + prior-2-day VA-trend bias (15m ONLY, CAUSAL).

Base : direction + A<=-0.75 (R-easy) + prev-same-dir + SKEW SIGN (skew>0 long | skew<0 short)  [LOOSER than the
       frozen |skew|>=0.4]. SL 0.1% beyond entry extreme; TP = RR x SL.
BIAS : L1 VA-trend ONLY (no breakout layer). Full-day 70% value area of D1 (2 days ago) vs D2 (yesterday):
...
Reports the baseline (looser skew, no bias), the +bias filtered set (controlled + one-at-a-time, per side), and the
HONEST partition (aligned-with-bias vs counter-bias) with a Fisher exact p. Compare the baseline here to the frozen
|skew|>=0.4 baseline to see the cost of loosening skew.

Run: python study/r15easy_vatrend_sk0.py
"""
...
def prep(A, first, dayva):
    ...
    for i in range(max(first, 1), len(A) - 1):
        ...
        if s == 0 or Aval[i] is None or Aval[i] > R_EASY or dirn[i - 1] != s:
            continue
        sk = A[i].get("sk")
        if sk is None or (sk <= 0.0 if s > 0 else sk >= 0.0):   # SKEW SIGN only: >0 long, <0 short
            continue
...
def main():
    ...
    print("=" * 112)
    print("15mReasy  SKEW-SIGN only (>0 L / <0 S)  + VA-trend bias  (15m)  |  signals %d (%dL/%dS)"
    ...
    for rr in RRS:
        for chain, vl in ((False, "controlled"), (True, "one-at-a-time")):
            ...
            print()
        print("HONEST (aligned vs counter-bias, chain):")
        honest(A, sigs, rr)
        print()
    print("CAVEAT: skew SIGN only (looser than frozen |skew|>=0.4). VA-trend causal. 15m, one regime, forward n=0.")
