"""Are 15m Radar Runner signals that fire INSIDE a 1h wall's radar area winners? (Mirror of the 4h-confluence test, one
level down.) Placebo-controlled (1h radars shifted to random levels) so 'inside a wide zone' isn't credited unless it
beats a random wide zone. Shipped spec: MINVISIT=1, candle-SL (15m buffer 0.3%) + fixed 0.5% TP. Both recon years.
Split OUTSIDE / INSIDE-1h / INSIDE-placebo -> n / win% / avg%net / exp-R + tier ladder P(>=1x/2x/3x). 3bps, 0.04% fee."""
...
def build_htf_walls(htf):
    A = sorted(load_archive(htf, root="study/recon_archive")[1], key=lambda b: _f(b.get("start_time", 0)))
...
def study(tf, walls, shifts):
    ...
    print("\n########  15m signals vs 1h walls  (total signals=%d)  ########" % len(rows), flush=True)
    for Y in (2025, 2026):
        yr = [x for x in rows if x[0] == Y]
        for gname, sel in (("OUTSIDE    ", lambda x: not x[4]), ("INSIDE-1h  ", lambda x: x[4]), ("INSIDE-plac", lambda x: x[5])):
...
def main():
    walls = build_htf_walls("1h")
    print("1h walls: %d" % len(walls), flush=True)
    shifts = [random.uniform(0.015, 0.035) * random.choice((-1, 1)) for _ in range(NPLAC)]
    study("15m", walls, shifts)
