"""'Enter the first time it prints' — test the most literal version: enter AT D the instant the fire prints
(causal, price-only entry cl[det]), holding through any later repaint, ZZTRAIL exit. Plus variants: D filled-only
(first-print causal tier), D + v2 zone filter (causal tier). All fully causal. Reference rows: enter-at-E and the
causal E2-tier replay (both already shown negative), and the settled look-ahead backtest. Three-outcome NET + t.
Run: python study/pivot_enter_first_print.py
"""
...
def main():
    ...
    for f in fires:
        ...
        buy = s == "long"
        p2d_c = spr(e_sh_c, det, buy)                             # FIRST-PRINT (causal) tier at D
        tier_c = "cyan/orange" if p2d_c > P2D_VHI else ("red/green" if p2d_c > P2D_HI else "hollow")
        gD = walk(det, det, buy) - FEE                            # enter AT D immediately
        zD = zone_at(det)
        take = None
        if zD is not None:                                       # v2 uses D-zone twice (entry==D here)
            take = take_rule(zD, buy, tier_c)
            if tier_c == "hollow" and (("buy" if buy else "sell"), zD, zD) in AVOID:
                take = False
        rows.append(dict(buy=buy, gD=gD, tier_c=tier_c, take=bool(take), has_ent=(ent is not None)))

    def show(tag, a):
        a = np.asarray(a)
        if not len(a):
            print("  %-32s n=0" % tag); return
        ...
        print("  %-32s n=%-3d | W %5.1f%% | BE %5.1f%% | L %5.1f%% | net %+.3f%% | TOT %+.2f%% ($%+.0f) | t=%+.2f"
              % (tag, nn, 100.0 * w / nn, 100.0 * b / nn, 100.0 * l / nn, a.mean(), a.sum(), a.sum() * 10.0, t))

    print("ENTER-AT-FIRST-PRINT (enter at D the instant it fires; causal, hold through repaint)\n")
    show("enter@D  ALL fires", [r["gD"] for r in rows])
    show("enter@D  filled-only (causal)", [r["gD"] for r in rows if r["tier_c"] != "hollow"])
    show("enter@D  hollow-only (causal)", [r["gD"] for r in rows if r["tier_c"] == "hollow"])
    show("enter@D  + v2 zone filter", [r["gD"] for r in rows if r["take"]])
    show("enter@D  longs", [r["gD"] for r in rows if r["buy"]])
    show("enter@D  shorts", [r["gD"] for r in rows if not r["buy"]])
