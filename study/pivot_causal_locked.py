"""v2 CAUSAL (live-tradeable) but take ONLY trades where the D is LOCKED by entry — i.e. the entry bar j0 lands
>= LOCK (7) buckets after detection, so the D's centered eff-agg window has fully settled and the D fill can no
longer repaint. Everything (tier/E-held/E2) uses the causal (left-clamped) eff-agg = the live view. Reports the
kept book overall + split by tier (both the CAUSAL first-print tier that routed the entry AND the now-settled
LOCKED tier the D shows once locked), vs the unfiltered v2 causal book. Three-outcome NET + t.
Run: python study/pivot_causal_locked.py
"""
...
LW = config.LIVE_PANEL_WINDOW; LOCK_LAG = LW // 2           # 7 — buckets for the D to settle
...
def main():
    ...
    def tier_of(p):
        return "cyan/orange" if p > P2D_VHI else ("red/green" if p > P2D_HI else "hollow")
    ...
    for f in fires:
        ...
        if ent is None:
            continue
        buy = s == "long"
        tier = tier_of(spr(e_sh_c, det, buy))                   # causal (first-print) tier
        ...
        if zD is not None and zE is not None:
            ...
            if tier == "hollow" and (("buy" if buy else "sell"), zD, zE) in AVOID:
                take = False
        rows.append(dict(g=walk(det, j0, buy) - FEE, tier=tier, tier_s=tier_of(spr(e_sh, det, buy)),
                         take=bool(take), locked=(j0 - det >= LOCK_LAG), lag=j0 - det))
    ...
    def show(tag, arr):
        ...
        print("    %-22s n=%-3d | W %2d (%5.1f%%) | BE %2d (%5.1f%%) | L %2d (%5.1f%%) | net %+.3f%% | TOT %+.2f%% ($%+.0f) | t=%+.2f"
              % (tag, nn, w, 100.0 * w / nn, b, 100.0 * b / nn, l, 100.0 * l / nn, a.mean(), a.sum(), a.sum() * 10.0, t))

    v2 = [r for r in rows if r["take"]]
    lk = [r for r in v2 if r["locked"]]; unlk = [r for r in v2 if not r["locked"]]
    print("v2 CAUSAL, D-LOCKED-by-entry filter (entry >= %d buckets after D)  [%d v2 trades: %d locked / %d not]\n"
          % (LOCK_LAG, len(v2), len(lk), len(unlk)))
    show("v2 causal ALL (ref)", [r["g"] for r in v2])
    show("v2 causal D-LOCKED", [r["g"] for r in lk])
    show("  (dropped: D not locked)", [r["g"] for r in unlk])
    print("\n  D-LOCKED book by CAUSAL first-print tier:")
    for t in ("hollow", "cyan/orange", "red/green"):
        show(t, [r["g"] for r in lk if r["tier"] == t])
    print("\n  D-LOCKED book by SETTLED (now-locked) tier — the reliable one you'd see:")
    for t in ("hollow", "cyan/orange", "red/green"):
        show(t, [r["g"] for r in lk if r["tier_s"] == t])
