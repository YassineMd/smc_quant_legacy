"""Hypothesis: CYAN/ORANGE Ds (the strongest D tier — aligned panel-2 spread at D > 80; cyan on buys, orange on
sells) are VERY effective when they land IN a 4h wick zone. Cross the D-fill tier (cyan/orange VHI > 80 |
green HI > 63 | hollow) with 4h in-zone (detection price within the GREEN buyer wick [low, vq_lo] long / RED
seller wick [vq_hi, high] short of the last completed 4h bucket). Three-outcome per cell (winner net>+0.05%% /
breakeven |net|<=0.05%% / loser net<-0.05%%). Frozen exit throughout. Run: python study/pivot_dtier_zone.py
"""
...
def main():
    ...
    for f in fires:
        ...
        rows.append((walk(det, j0, buy), tier, inz, buy))

    G = np.array([r[0] for r in rows]); NET = G - FEE
    TIER = np.array([r[1] for r in rows]); INZ = np.array([True if r[2] else False for r in rows])
    HAS4H = np.array([r[2] is not None for r in rows])
    d = 1000.0 / 100.0
    ...
    def line(tag, m):
        ...
        print("  %-26s n=%-3d | W %2d (%4.1f%%) | BE %2d (%4.1f%%) | L %2d (%4.1f%%) | net %+.3f%% | TOT %+.2f%% ($%+.0f)"
    ...
    cyan = TIER == "cyan"; green = TIER == "green"; hollow = TIER == "hollow"
    print("CYAN/ORANGE D x 4h ZONE  (n=%d, of which %d have a completed 4h bucket at D)\n" % (len(rows), int(HAS4H.sum())))
    print("  D-TIER headline:")
    line("cyan/orange (VHI >80)", cyan); line("green (HI 63-80)", green); line("hollow (<=63)", hollow)

    print("\n  THE HYPOTHESIS — cyan/orange split by 4h zone:")
    line("cyan/orange  IN-ZONE", cyan & HAS4H & INZ)
    line("cyan/orange  OUT-ZONE", cyan & HAS4H & ~INZ)

    print("\n  reference cells:")
    line("ALL IN-ZONE", HAS4H & INZ)
    line("green IN-ZONE", green & HAS4H & INZ)
    line("hollow IN-ZONE", hollow & HAS4H & INZ)
