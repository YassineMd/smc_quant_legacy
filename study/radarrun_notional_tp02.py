"""Scenario: OPTIMAL-rule stop (candle-capped ~1%) but NOTIONAL 10%x10 (f=1.0) sizing and TP 0.20%. Same table as
radarrun_two_scenarios (win%, DD, days-to-pass, trades-to-pass, pass%). RR-only 15c+30c+30bkt, higher-TF barrier,
HyroTrader $200k target10/max6-trail/daily-trail, 20k paths. IN-SAMPLE. python study/radarrun_notional_tp02.py"""
...
FEE, SLIP, TP, H = 0.0004, 0.0003, 0.0020, 200          # TP 0.20%
...
def main():
    print("RadarRun — candle stop (~1%%) + NOTIONAL 10%%x10 + TP 0.20%% | RR 15c+30c+30bkt | HyroTrader $200k | 20k | IN-SAMPLE\n", flush=True)
    dets = load_dets()
    tr = eval_candle(dets)
    nets = np.array([t[1] for t in tr]) * 100.0
    w = nets[nets > 0]; l = nets[nets <= 0]
    print("  win rate %.1f%%  |  avg win %+.3f%%  avg loss %+.3f%%  worst-trade %+.3f%%  |  expectancy %+.4f%%/trade  |  %d signals"
          % (100.0 * (nets > 0).mean(), w.mean(), l.mean(), nets.min(), nets.mean(), len(tr)), flush=True)
