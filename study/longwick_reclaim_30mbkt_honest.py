"""LONG WICK RECLAIM — HONEST test, 30m BUCKET (user 2026-08-25). Same pre-registration family as the Long Wick
runs: signals = the SHIPPED app/longwick_detect.detect_reclaim (2-bar failed-push pair, NO wall binding — pure
...
recon per-year + DAEMON OOS; ALL/LONG/SHORT. NO iteration. python study/longwick_reclaim_30mbkt_honest.py"""
import os, sys, time
...
def signals(daemon):
    ...
    marks = LW.detect_reclaim(A, skip_last=False)
    fires = []
    for e in marks:
...
def main():
    from study.archive_loader import load_archive
    from study.candle_bias_1h import _f
    print("LONG WICK RECLAIM — 30m BUCKET HONEST TEST | shipped detect_reclaim (wall-free, causal) | SL 0.1% beyond"
