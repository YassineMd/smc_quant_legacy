"""MMXSKEW v1.3 gate — FORWARD AUDIT (periodic check; mirrors study/mmxskew_v12_forward_audit.py).

Runs the frozen v1.3 gate (v1.1 minus POC + mov_mag>=39 + raw da2>0 asymmetric — see study/mm_skew_v13_validate.py)
on the current tape, splits each TAKEN trade at freeze_ts (bootstrapped on the FIRST run = the newest in-sample
taken entry), and prints in-sample vs FORWARD n/W/L/net/exp/t per RR, a verdict, and appends one dated row to
study/out/mmxskew_v13_forward_log.md.

v1.3 is a CANDIDATE — in-sample n=23-26, mined, one regime — so THIS is what decides it. NEVER re-tune (any change
= a NEW freeze: delete the json + log). Pull a fresh snapshot/archive first (study/pull_archive.ps1) so forward
bars — and their 1m sub-buckets / delta_h1 needed for da2 — exist.

Pre-declared: PASS = fwd n>=20 & net>0 & t>=1.5 ; FAIL = fwd n>=20 & net<=0 ; degrade-warn = fwd n>=15 & exp<=0.
Run: python study/mmxskew_v13_forward_audit.py
"""
...
FREEZE = os.path.join(HERE, "out", "mmxskew_v13_freeze.json")
LOG = os.path.join(HERE, "out", "mmxskew_v13_forward_log.md")
...
def stats(trs):
    ...
    nets = np.array([t["net"] * 100.0 for t in trs], float)   # v13.taken stores net as a fraction (fee-in)
...
def main():
    A, sigs = V13.build()
    trades = {rr: V13.taken(A, sigs, rr) for rr in (1.0, 1.5)}
    ...
    if not os.path.exists(FREEZE):
        ...
        fz = dict(freeze_ts=ft, freeze_utc=datetime.datetime.utcfromtimestamp(ft).strftime("%Y-%m-%d %H:%M:%S UTC"),
                  gate="v1.1 minus POC + mov_mag>=39 + raw da2>0 (asymmetric)",
    ...
    print("MMXSKEW v1.3 gate forward audit  |  freeze_ts %d (%s)" % (int(ft), fz["freeze_utc"]))
    row_bits = []
    for rr in (1.0, 1.5):
    ...
    with open(LOG, "a", encoding="utf-8") as f:
        if fresh:
            f.write("# MMXSKEW v1.3 gate - forward log (append-only)\n\n")
            f.write("Gate: v1.1 minus POC + mov_mag>=39 + raw da2>0 (asymmetric). Freeze_ts %d = %s.\n"
                    % (int(ft), fz["freeze_utc"]))
            for rr in ("1.0", "1.5"):
                b = fz["baseline"][rr]
                f.write("  RR 1:%s in-sample n=%d win %.1f%% exp %+.3f%%/tr (MINED, n<30).\n" % (rr, b["n"], b["win"], b["exp"]))
