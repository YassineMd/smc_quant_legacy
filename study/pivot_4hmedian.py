"""4h-median mean-reversion filter on the FROZEN strategy: take LONG only if price is BELOW the last COMPLETED
4h (240x) whiskerplot bucket's MEDIAN (vq_med), SHORT only if ABOVE it. Uses the last fully-formed 4h bucket
(bisect on end_time), NOT the current forming one. Median = bar_quantiles.vq(levels)[1] of the tf='4h' stored
buckets. Compares filtered vs frozen base (win%/net/total, loser/winner cut). Tests det-price and entry-price.
Run: python study/pivot_4hmedian.py
"""
...
def load_4h_medians():
    ...
    for db in sorted(glob.glob(os.path.join(REPO, "study", "data", "history_snapshot_*.db"))):
        ...
        for (x,) in con.execute("SELECT data FROM closed_buckets WHERE tf='4h' ORDER BY id"):
            b = json.loads(x)
            if b.get("levels"):
                by[float(b["end_time"])] = b                # dedup by end_time across snapshots
        con.close()
    b4 = [by[k] for k in sorted(by)]
    z_et = [float(b["end_time"]) for b in b4]
    z_med = [float(bar_quantiles.vq(b["levels"])[1]) for b in b4]   # [1] = median
    return z_et, z_med
...
def main():
    ...
    scan = {"long": 0, "short": 0}; rows = []      # (gross, buy, det, j0)
    for f in fires:
        ...
        if j0 is None:
            continue
        rows.append((walk(det, j0, buy), buy, det, j0))

    def med_at(bar):
        i = bisect.bisect_right(z_et, et[bar]) - 1          # last COMPLETED 4h bucket
        return z_med[i] if i >= 0 else None

    def report(name, price_bar_idx):
        keep = []; drop = []
        for g, buy, det, j0 in rows:
            m = med_at(det if price_bar_idx == "det" else j0)
            if m is None:
                continue
            px = float(cl[det if price_bar_idx == "det" else j0])
            ok = (px < m) if buy else (px > m)              # long below median / short above
            (keep if ok else drop).append(g)
        base = np.array([r[0] for r in rows]); k = np.array(keep) if keep else np.array([0.0])
        d = 1000.0 / 100.0
        print("  filter on %s price:" % name)
        print("    BASE (all)         : n=%-3d win %.1f%% net %+.3f%% | TOTAL %+.2f%% ($%+.0f)"
              % (len(base), 100.0 * np.mean(base > 0), (base - FEE).mean(), (base - FEE).sum(), (base - FEE).sum() * d))
        print("    KEEP (rule passes) : n=%-3d win %.1f%% net %+.3f%% | TOTAL %+.2f%% ($%+.0f)"
              % (len(keep), 100.0 * np.mean(k > 0), (k - FEE).mean(), (k - FEE).sum(), (k - FEE).sum() * d))
        dr = np.array(drop) if drop else np.array([])
        if len(dr):
            print("    (dropped %d: win %.1f%% net %+.3f%%)" % (len(dr), 100.0 * np.mean(dr > 0), (dr - FEE).mean()))

    print("4h-MEDIAN mean-reversion filter (long below / short above last completed 4h median), %d trades\n" % len(rows))
