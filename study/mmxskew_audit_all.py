...
Lines:
  ...
  v1.2-Dynamic — v1.1 + run_pos<=4 + mov_mag_ratio>=1.25
...
def get_freeze(trades, path, gate_desc):
    if not os.path.exists(path):
        ...
        fz = dict(freeze_ts=ft, freeze_utc=datetime.datetime.utcfromtimestamp(ft).strftime("%Y-%m-%d %H:%M:%S UTC"),
                  gate=gate_desc, baseline={str(rr): stats(trades[rr]) for rr in (1.0, 1.5)},
