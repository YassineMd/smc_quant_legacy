"""Three-outcome reporting convention (winner / breakeven / loser), split by 4h in-zone. Every trade is
classified by NET PnL (gross - 0.10%% fee):
    WINNER    net >  +0.05%%
    BREAKEVEN |net| <= 0.05%%   (the +0.40%%->+0.10%% breakeven-lock scratches land here: +0.10%% gross = 0 net)
    LOSER     net <  -0.05%%
In-zone = detection price within the GREEN buyer wick [low, vq_lo] (long) / RED seller wick [vq_hi, high]
(short) of the last completed 4h bucket. Frozen exit throughout. Run: python study/pivot_outcomes3.py
"""
...
TRAIL = 0.0005; SL_PAD = 0.001; ARM = 0.0040; LOCK = 0.0010; BE = 0.05     # breakeven half-band (net %)
...
def main():
    ...
    for f in fires:
        ...
        rows.append((walk(det, j0, buy), inz))

    G = np.array([r[0] for r in rows]); NET = G - FEE; INZ = np.array([r[1] for r in rows])
    d = 1000.0 / 100.0
    win = NET > BE; be = np.abs(NET) <= BE; los = NET < -BE

    hdr = "  %-9s | %-4s | %-18s | %-13s | %-18s | %-9s" % (
        "group", "n", "WINNERS", "BREAKEVEN", "LOSERS", "TOTAL")
    print("THREE-OUTCOME table  (winner net>+%.2f%% | breakeven |net|<=%.2f%% | loser net<-%.2f%%),  %d trades\n"
          % (BE, BE, BE, len(rows)))
    print(hdr); print("  " + "-" * (len(hdr) - 2))

    def row(tag, m):
        g = NET[m]; w = win[m]; b = be[m]; l = los[m]
        nn = max(1, len(g))
        aw = g[w].mean() if w.any() else 0.0; al = g[l].mean() if l.any() else 0.0
        print("  %-9s | %-4d | %2d (%4.1f%%) avg %+.2f%% | %2d (%4.1f%%) | %2d (%4.1f%%) avg %+.2f%% | %+.2f%% ($%+.0f)"
              % (tag, len(g), int(w.sum()), 100.0 * w.sum() / nn, aw,
                 int(b.sum()), 100.0 * b.sum() / nn,
                 int(l.sum()), 100.0 * l.sum() / nn, al, g.sum(), g.sum() * d))

    row("ALL", np.ones(len(G), bool)); row("IN-ZONE", INZ); row("OUT-ZONE", ~INZ)
