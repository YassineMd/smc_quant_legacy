"""v2 on the CAUSAL (live-tradeable) tape, with an EXTRA filter: take the trade only if the P2 HARMONIC-MEAN
SPREAD is IN FAVOUR of the position — a GREEN (net-bull) HM spread for a long, RED (net-bear) for a short. The HM
spread mirrors the terminal box exactly: the harmonic mean of the per-bar dominant eff-agg share over the LAST 3
LOCKED cycles as-of the entry, expressed as a spread (0-100), with the net-dominant side giving the colour.

Causal / no look-ahead: a "locked" cycle is one whose closing cross is >= LOCK (7) buckets behind the entry j0, so
its centered eff-agg window is fully settled and available at trade time — the HM box is computed only from those
settled bars. Entry routing (tier / E-held / E2 / zone filter + hollow avoid) uses the causal (left-clamped) share
exactly as the live indicator would. Reports three-outcome NET + t for: v2 causal ALL, v2 + HM-favour, the dropped
(HM against / no locked cycles), and the HM-favour book split by the causal D-tier.
Run: python study/pivot_causal_hmfavor.py
"""
...
LW = config.LIVE_PANEL_WINDOW; LOCK_LAG = LW // 2          # 7 — buckets for the P2 centered window to settle
...
def main():
    ...
    n = len(bks)
    _, e_sh, _, _ = PD._p9_global(snaps); e_sh = np.asarray(e_sh, float)   # SETTLED centered P2 share (= terminal P2)
    ...
    e_sh_c = causal_share(eb, er_, LW)                                     # CAUSAL (first-print) share — routes entry
    ...
    # P2 cycles on the SETTLED share (a cycle = a run one side of 50%, bounded by crossings). A cycle is USABLE for
    # a trade at j0 only if its closing bar is >= LOCK behind j0 (fully settled = causal). Precomputing on the full
    # settled series is identical to detecting up to j0 for any cycle that ends <= j0-LOCK (its bars are all settled).
    CYC = []; i0 = 0; dom = e_sh[0] >= 0.5
    for k in range(1, n):
        dk = e_sh[k] >= 0.5
        if dk != dom:
            CYC.append((i0, k - 1)); i0 = k; dom = dk
    CYC.append((i0, n - 1))
    cyc_end = np.array([c[1] for c in CYC])

    def hm_favor(j0, buy):
        """Terminal HM box, as-of the entry j0: HM of the per-bar dominant SETTLED share over the last 3 LOCKED
        cycles (closing cross < j0-LOCK). Returns (in_favour, spread, net_bull) or None if no locked cycle yet."""
        lockidx = j0 - LOCK_LAG
        m = int(np.searchsorted(cyc_end, lockidx, side="left"))   # # of cycles with end < lockidx (locked)
        if m == 0:
            return None
        l3 = CYC[max(0, m - 3):m]                                 # last <=3 locked cycles
        s0 = l3[0][0]; s1 = l3[-1][1]; seg = e_sh[s0:s1 + 1]
    ...
    scan = {"long": 0, "short": 0}; rows = []; no_lock = 0
    for f in fires:
        ...
        if ent is None:
            continue
        buy = s == "long"
        p2d = spr(e_sh_c, det, buy)                              # CAUSAL tier (first-print)
        ...
        if hv is None:
            no_lock += 1
        favor = bool(hv is not None and hv[0])
        rows.append(dict(g=walk(det, j0, buy) - FEE, tier=tier, take=bool(take),
                         favor=favor, has_hm=hv is not None))
    ...
    against = [r for r in v2 if r["has_hm"] and not r["favor"]]
    nohm = [r for r in v2 if not r["has_hm"]]
    print("v2 CAUSAL + P2 HM-SPREAD-in-favour filter  (last 3 LOCKED cycles; %d v2 trades)\n" % len(v2))
    show("v2 causal ALL (ref)", [r["g"] for r in v2])
    show("v2 + HM in favour", [r["g"] for r in fav])
    show("  dropped: HM against", [r["g"] for r in against])
    show("  dropped: no locked cycle", [r["g"] for r in nohm])
    print("\n  HM-in-favour book by CAUSAL D-tier:")
    for t in ("hollow", "cyan/orange", "red/green"):
        show(t, [r["g"] for r in fav if r["tier"] == t])
    print("\n  kept %d / %d v2 trades (%d against, %d no-locked-cycle)"
          % (len(fav), len(v2), len(against), len(nohm)))
