"""BREAKOUT-CLOSE x RADAR RUNNER, 30m bucket — HONEST test (user 2026-08-27: LONG only if the signal bar
CLOSES above the PREVIOUS bar's high; SHORT only if it closes below the previous bar's low). Canonical
harness: cached union fire sets, 1-MINUTE first-touch, non-overlap taken(), fees 0.04% RT + 0.03% slip/leg,
prop MC; recon per-year + DAEMON OOS decisive.

PRE-REGISTERED (frozen; no iteration): on the SAME 30m series the fires index into, fire at bar b is
WITH-BC iff (s>0 and C[b] > H[b-1]) or (s<0 and C[b] < L[b-1]); NO-BC = the complement (the side's
breakout-close condition not met). b=0 or a degenerate previous bar -> excluded from conditioned cells.
CELLS: ALL control / WITH-BC (the rule) / NO-BC / WITH-LONG / WITH-SHORT.
EXITS: 0.2% net, 0.4% net, RR 1:0.5, RR 1:1.
python study/radarrun_brkclose_30m.py"""
...
def features(fires, A):
    ...
    for f in fires:
        b = int(f[0]); s = int(f[2])
        if b < 1 or H[b - 1] <= 0 or L[b - 1] <= 0 or C[b] <= 0:
            nex += 1
            recs.append(dict(f=tuple(f), s=s, with_bc=False, no_bc=False))
            continue
        bc = (s > 0 and C[b] > H[b - 1]) or (s < 0 and C[b] < L[b - 1])
        recs.append(dict(f=tuple(f), s=s, with_bc=bc, no_bc=not bc))
    print("  (no previous bar / degenerate: %d fires excluded from conditioned cells)" % nex, flush=True)
    return recs


CELLS = [
    ("ALL", lambda r: True),
    ("WITH-BC", lambda r: r["with_bc"]),
    ("NO-BC", lambda r: r["no_bc"]),
    ("WITH-LONG", lambda r: r["with_bc"] and r["s"] > 0),
    ("WITH-SHORT", lambda r: r["with_bc"] and r["s"] < 0),
]
...
def main():
    ...
    print("BREAKOUT-CLOSE x RADAR RUNNER 30m BUCKET — canonical harness | long: C > prev high / short: C < prev low | pre-registered\n", flush=True)
    t0 = time.time()
