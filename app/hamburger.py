...
_M10_INDICATORS = [
    ...
    ("m10_daycompass", "★ Day Compass (bias table)", False, True),   # BOTTOM-LEFT HUD: price vs YESTERDAY's 70% value area (ABOVE/INSIDE/BELOW) + TODAY's wall ledger (S/R created - mitigated) -> one needle: UP BIAS / DOWN BIAS / ROTATION / MIXED. DESCRIPTIVE + COINCIDENT; the pre-registered RadarRun study is the referee. app/day_compass
...
_M10_STRATEGIES = [
    ...
    ("m10_wallsurge", "Wall Surge (▲▼ · strong Δ+Vol + kept≥80% @ 30m wall · 1m·5m clock)", False, True),  # 1m/5m CLOCK only: pane-STRONG |delta| AND pane-STRONG total volume (each trailing-50 pct ≥ P80) AND Eff/Res retention ≥ 80% (kept its delta-direction excursion) inside a same-side 30m wall CORE — strong held buying on a support wall ▲ / strong held selling on a resistance wall ▼. Descriptive/eyeball, no tested edge
