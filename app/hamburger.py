...
class FloatingOverlayMenu(QtWidgets.QFrame):
    ...
    def _build_hlh_subtoggles(self, section) -> None:
        ...
        (the Pine's rule: turning a table off never changes the blocs, the merges or the colours), and so is
        'POC acceptance areas', which is ours rather than the Pine's. Defaults as the Pine: week / bloc-only
        OFF, badges / tables ON; the acceptance areas follow config.HLH_SHOW_POC_RUNS so the default lives in
        one place instead of being written out twice."""
    ...
    def _build_bigplayer_sweeps_subtoggle(self, section) -> None:
        ...
        cb = QtWidgets.QCheckBox("· Sweeps / campaigns (one player, ≤30 ms)")
        cb.setChecked(True)
        cb.setStyleSheet("QCheckBox{ padding-left:18px; color:#aeb4c0; font-size:10px; }")
        cb.setToolTip("One player working the book: a market order eating several levels (its fills share the "
                      "exact same millisecond and side), or same-side prints / sweeps within 30 ms of the previous same-side one (the other side in between and refills taken again allowed). "
    ...
    def _build_flow_section(self, root) -> None:
        ...
        self.fratio_on.setToolTip(
            ...
            "the pane's top right shows one series instead: Buyer Ratio, Seller Ratio, or Delta Ratio (the size "
            "of the cycle's net buy-minus-sell $ per second against the previous cycles', teal when buyers were "
