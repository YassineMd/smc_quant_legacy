...
_M10_STRATEGIES = [
    ...
    ("m10_longwick_combo", "Long Wick Combo (gold ♦ · 2-bar failed push + break · no wall · all tf)", False, True),  # gold ♦ ABOVE a BEARISH bar + long-UPPER-wick BEARISH bar (v2 wick geometry) whose CLOSE is BELOW the prev candle's LOW — buyers pushed, completely failed, bar still broke down; mirrored bullish pair closing ABOVE the prev high -> gold ♦ BELOW. NOT bound to walls. Descriptive/eyeball  # red ♦ ABOVE a BEARISH bar at a SELL wall whose upper wick > body AND >= 2x the lower wick (the lower wick may be any size, even > body, as long as it's doubled); green ♦ BELOW the bullish mirror at a BUY wall. Walls = the chart's own current-tf Order-Flow Walls. Descriptive/eyeball — the v1 geometry's mechanical bracket FAILED the honest test on 15m/30m/1h; v2 untested  # 1m/5m CLOCK only, pane-STRONG |delta| AND volume (each trailing-50 pct ≥ P80) at a 30m OR 1h BUCKET wall CORE. TWO classes: SURGE = aggressor wins at its own wall (kept≥80% · buying on support ▲ / selling on resistance ▼); ABSORB = aggressor loses at the opposing wall (strong selling on support + GREEN close ▲ / strong buying on resistance + RED close ▼). Descriptive/eyeball, no tested edge
