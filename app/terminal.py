...
class MinimalTerminalWindow(QtWidgets.QMainWindow):
    ...
    def _draw_absorb_levels(self, buckets, x, vx0, vx1) -> None:
        ...
        for _mid_idx, m in enumerate(self._absorblvl_marks):
            ...
            if s < self._wall_floor:                            # strength draw floor (hamburger slider)
                continue
            src = m.get("src", "")
    ...
    def _draw_crazy_wall(self, filtered) -> None:
        ...
        show_pa = self.menu.layer_state("m10_wallabs_pureagg")    # ▋ green/red one-sided-aggression label
        n = len(filtered)
        if n < 2 or not (show_crazy or show_big or show_eg or show_pa):
            self._clear_crazy_wall(); return
        _sig = (n, filtered[-1].get("end_time"), filtered[-1].get("close"), show_crazy, show_big, show_eg, show_pa)
        ...
        self._draw_pure_agg(filtered, marks, show_pa, vx0, vx1, pad)    # ▋ one-sided-aggression labels (wall MARKS)
        u = 0
        for h in hits:
    ...
    def _draw_pure_agg(self, filtered, marks, show, vx0, vx1, pad) -> None:
        ...
        if not show or n < 2:
            ...
            return
        try:
            from app import pure_aggression_detect
            pas = pure_aggression_detect.from_walls(filtered, marks)
        except Exception:
            for _t in self._pureagg_pool:
                _t.setVisible(False)
            return
        pad2 = pad * 1.9                                       # sit a touch further out than the Easy Gold badge
        u = 0
        for g in pas:
            i = int(g["i"])
            if i < 0 or i >= n or i < vx0 - 1.0 or i > vx1 + 1.0:   # cull to viewport
    ...
    def _refresh_selection_stats(self) -> None:
        ...
        if rect is None or self.scanner_mode != "bucket_canvas":
            ...
            return
        self._nosel_sig = None            # a selection is active -> re-run the no-selection path once on teardown
        filtered, _x, _a = self._build_scanner_buckets()
        if not filtered:
            self.sel_stats.hide()
            self._hide_sel_ctrls()
